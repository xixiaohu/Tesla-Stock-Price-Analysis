...
import time
from dash.dependencies import Output,  Event, Input #event is kind of inputs
import plotly
...
# if today is Saturday, graph will show Friday's price
if day == 5:
    stamp1 = open_stamp_y
...
# if today is Sunday, graph will show Friday's price
elif day == 6:
...
# if today is Monday    
    ...
    # if current time is before open time of Monday, graph will show Friday's price 
    if now_stamp < open_stamp:
        ...
        fdate = datetime.datetime.fromtimestamp(open_stamp_f).strftime("%Y-%m-%d %H:%M:%S")
        stockdate = fdate[0:10]
    # if current time is after close time of Monday, graph will show today's (Monday's) price fluctuantion
    elif now_stamp > close_stamp:
        stamp1 = open_stamp
        stamp2 = close_stamp
        live = False
        stockdate = todaydate
    # If current time is in open time, the graph will be lively updated
    else:
        stamp1 = open_stamp
        stamp2 = now_stamp
        live = True
...
# if today is Tuesday, Wednesday, Thursday and Friday 
else:
    # if current time is before open time, graph will show yesterday's pric
    if now_stamp < open_stamp: 
        stamp1 = open_stamp_y
        stamp2 = close_stamp_y
        live = False
    ...
    # if current time is after close time, graph will show today's price fluctuantion
    elif now_stamp > close_stamp:
        #whole day
        stamp1 = open_stamp
        stamp2 = close_stamp
        live = False
        stockdate = todaydate
    # If current time is in open time, the graph will show price fluctuation from open time up to now and update lively
    else:
        #up-to-now
        stamp1 = open_stamp
        stamp2 = now_stamp
...
#******************************************************************************************************************************************************************************************************************************************************************************************************************
# SQL to store time stamp and stock price into a database for later queries
...
#******************************************************************************************************************************************************************************************************************************************************************************************************************
# nlp, parse sentences for wordcloud, tfidf and simple sentiment analysis
ps = PorterStemmer()
...
#******************************************************************************************************************************************************************************************************************************************************************************************************************
# WordCloud, generate a wordcloud picture from today's news and save it to local folder  
contents = []
for header in news.keys():
    content = news[header]
...
#******************************************************************************************************************************************************************************************************************************************************************************************************************
# idf, calculate tfidf of the key word "Tesla" and sort news according to the score
...
# sort news by tfidf number shows only the header
...
#******************************************************************************************************************************************************************************************************************************************************************************************************************
# predict price, using 3 SVR models to predict next 5 days prices from last 30 days' prices
...
train_price = dfp.close.values 

n = len(train_price)
pastdays = list(range(0, n)) 
nx = list(range(n,n+5)) 
...
#******************************************************************************************************************************************************************************************************************************************************************************************************************
# dash app, create an interactive dashboard using dash 
...
app.layout = html.Div([
    ...
    dcc.Tabs(id="tabs", children=[
        # tab1 of stock price, live-updated price graph and historical price graph
        dcc.Tab(label='Stock Price', style=tab_style, selected_style=tab_selected_style, children=[
		    ...
		    # user could choose 4 indicators, volume is shown as area under line graph
		    # user could choose data from 4 periods: 1 month, 6 months, 1 year and 3 years
                    html.Div([
                        dcc.Dropdown(
                            id='yaxis-indicators',
        ...
        # tab2 of news, sorted news list with hyperlink, wordcloud, sentiment analysis
        ...
        dcc.Tab(label='Prediction', style=tab_style, selected_style=tab_selected_style, children=[
            ...
            # tab3 predictive models, evaluation of predictive models and predcited prices of next 5 days
            html.Div([
                html.Div([
